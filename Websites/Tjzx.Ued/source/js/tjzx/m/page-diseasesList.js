...
(function ($, T) {
    //初始化 疾病分科
    T.getJson("/m/diseases/department_list", {
        state: 1
    }, function (json) {
        var list = json.data.list;
        if (json.state && list && list.length > 0) {
            var $sels = $("select[name='diseasesDepartmentId'],select[name='CategoryId']");
            var html = "";
    ...
    var getList = function (page) {
        ...
        T.getJson("/m/diseases/list", data, function (json) {
            if (json.state) {
    ...
    var loadItem = function (id) {
        ...
        T.getJson("/m/diseases/item", {
            id: id
        }, function (json) {
            if (!json.state) {
                T.msg(json.msg || "获取疾病信息失败！");
                return false;
            }
            $(".m-panel-title li").removeClass("active");
            $(".m-panel-title ul").append("<li class=\"m-panel-add active\">编辑疾病</li>");
            $(".j-back").show();
            vForm.bind(json.data);
            editor.setContent(json.data.diseasesDescription);
    ...
    $(document)
        ...
        .delegate(".j-submit", "click", function () {
            ...
            formData.diseasesDescription = content;
            T.getJson("/m/diseases/add", formData, function (json) {
                if (json.state) {
                    T.msg(formData.diseasesId > 0 ? "编辑成功！" : "添加成功！", "reload");
                } else {
                    T.msg(json.msg || "操作异常！");
        ...
        .delegate(".j-state", "click", function () {
            ...
            T.getJson("/m/diseases/updateState", {
                id: id,
                state: state
            }, function (json) {
                if (json.state) {
        ...
        .delegate(".j-delete", "click", function () {
            var id = $(this).parents("tr").data("id");
            if (!confirm("确认删除该疾病？")) return false;
            T.getJson("/m/diseases/delete", {
                id: id
            }, function (json) {
                if (json.state) {
                    T.msg(json.msg || "删除成功！");
        ...
        .delegate(".j-restore", "click", function () {
            var id = $(this).parents("tr").data("id");
            if (!confirm("确认还原该疾病？")) return false;
            T.getJson("/m/diseases/restore", {
                id: id
            }, function (json) {
                if (json.state) {
                    T.msg(json.msg || "还原成功！");
