<%@ WebHandler Language="C#" Class="Tjzx.Official.Web.GetRemoteImage" %>
...
namespace Tjzx.Official.Web
{
    public class GetRemoteImage : IHttpHandler
    {
        ...
        {
            const string savePath = "/upload/"; //保存文件地址
            string[] filetype = { ".gif", ".png", ".jpg", ".jpeg", ".bmp" };             //文件允许格式
            const int fileSize = 3000; //文件大小限制，单位kb
            ...
            try
            {
                for (int i = 0, len = imgUrls.Length; i < len; i++)
                {
                    ...
                    //写入文件
                    filename = filepath + Guid.NewGuid() + currentType;
